...
public class SimManager extends SimEntity {
	...
	public SimManager(ScenarioFactory _scenarioFactory, int _numOfMobileDevice, String _simScenario, String _orchestratorPolicy) throws Exception {
		...
		//Create Physical Servers
		droneServerManager = ((MyScenarioFactory)(scenarioFactory)).getDroneServerManager();
		droneServerManager.initialize();
			...
			switch (ev.getTag()) {
				...
				case GET_LOAD_LOG:
					SimLogger.getInstance().addVmUtilizationLog(
							...
							droneServerManager.getAvgUtilization(),
				...
				case MOVE_DRONES:
					...
					if (CloudSim.clock() < SimSettings.getInstance().getSimulationTime())
						schedule(getId(), SimSettings.getInstance().getSimulationTime() / 10, MOVE_DRONES);
					break;
				case PRINT_PROGRESS:
	...
	public void shutdownEntity() {
		...
		droneServerManager.terminateDatacenters();
