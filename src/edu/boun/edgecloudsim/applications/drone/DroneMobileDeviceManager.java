...
public class DroneMobileDeviceManager extends MobileDeviceManager {
	...
	public DroneMobileDeviceManager() throws Exception{
	}
	...
	public UtilizationModel getCpuUtilizationModel() {
		return new DroneCpuUtilizationModel();
	}
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		DroneEdgeOrchestrator edgeOrchestrator = (DroneEdgeOrchestrator)SimManager.getInstance().getEdgeOrchestrator();

		Task task = (Task) ev.getData();

		SimLogger.getInstance().taskExecuted(task.getCloudletId());

		if(task.getAssociatedDatacenterId() == DroneEdgeOrchestrator.CLOUD_DATACENTER_VIA_GSM) {
		...
		else if(task.getAssociatedDatacenterId() == DroneEdgeOrchestrator.CLOUD_DATACENTER_VIA_RSU) {
		...
		else if(task.getAssociatedDatacenterId() == DroneEdgeOrchestrator.EDGE_DATACENTER) {
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		{
			((DroneNetworkModel)networkModel).updateMM1QueeuModel();
		...
		case READY_TO_SELECT_VM:
		{
			...
			if(nextHopId == DroneEdgeOrchestrator.CLOUD_DATACENTER_VIA_GSM){
				nextEvent = REQUEST_RECEIVED_BY_CLOUD;
				vmType = VM_TYPES.CLOUD_VM;
			}
			//task is sent to cloud over RSU via 2 hops
			else if(nextHopId == DroneEdgeOrchestrator.CLOUD_DATACENTER_VIA_RSU){
			...
			else if(nextHopId == DroneEdgeOrchestrator.EDGE_DATACENTER){
	...
	private void submitTaskToVm(Task task, SimSettings.VM_TYPES vmType) {
		...
		double requiredCapacity = ((DroneCpuUtilizationModel) task.getUtilizationModelCpu()).predictUtilization(vmType);
	...
	private Task createTask(TaskProperty edgeTask){
		...
		if (utilizationModelCPU instanceof DroneCpuUtilizationModel) {
			((DroneCpuUtilizationModel)utilizationModelCPU).setTask(task);
		}
