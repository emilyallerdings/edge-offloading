...
public class DroneVmAllocationPolicy extends VmAllocationPolicy {
    /** The vm table. */
    private Map<String, Host> vmTable;
    private static int createdVmNum;
    private int DataCenterIndex;

    public DroneVmAllocationPolicy(List<? extends Host> list, int _DataCenterIndex) {
        super(list);
    ...
    public boolean allocateHostForVm(Vm vm) {
        ...
        if (!getVmTable().containsKey(vm.getUid()) && vm instanceof DroneVM) { // if this vm was not created
            boolean vmFound = false;
            int vmCounter = SimSettings.getInstance().getNumOfEdgeVMs();
            int hostIndex = (vm.getId() - SimSettings.getInstance().getNumOfEdgeVMs()) / 2;
