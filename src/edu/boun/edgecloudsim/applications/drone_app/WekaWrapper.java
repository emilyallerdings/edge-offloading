	...
	public double handleRegression(int targetDatacenter, double[] values) {
		...
		try {
			if(targetDatacenter == MyEdgeOrchestrator.EDGE_DATACENTER) {
				...
				Instance data = getRegressionData("drone", values,
						DRONE_REGRESSION_ATTRIBUTES,
						DRONE_REGRESSION_MEAN_VALS,
						DRONE_REGRESSION_STD_VALS);
	...
	public boolean handleClassification(int targetDatacenter, double[] values) {
		...
		try {
			if(targetDatacenter == MyEdgeOrchestrator.EDGE_DATACENTER) {
				...
				Instance data = getClassificationData("drone", values,
						DRONE_CLASSIFIER_ATTRIBUTES,
						DRONE_CLASSIFIER_MEAN_VALS,
						DRONE_CLASSIFIER_STD_VALS);
